# custom version of eval.py
# run like: python3 scripts/eval_stan.py --dir output/expr_20241028_005137/ --ckpt_num 825 --num_rollouts 38 --skip_rollouts 37
...
def get_args():
    ...
    parser.add_argument('--skip_rollouts', type=int, default=0,
                        help="Number rollouts to skip")
...
def run_rollouts(agent, env, log_dir, num_rollouts=1, skip_rollouts=0, render=False):
    """
    ...
    with jsonlines.open(log_dir, "w") as writer:
        for i in tqdm.tqdm(range(num_rollouts)):
            ...
            # Get initial info using the environment's generate_info() method
            

            # print env attributes (attr)
            #print(f"env attributes: {env.__dict__.keys()}")

            writer.write(state)

            while not done:
                # progress environment state
                prev_obs = obs.tolist().copy()
...
def main():
    ...
    with open(ray_config_path, 'rb') as ray_config_f:
        ...
        # STAN: manual settings override here since using --alt_env_config failed for me 
        print("STAN NOTE (eval.py): hardcoded settings override for lead x/y and wingman radius")
        # lead
        ray_config['env_config']['env_objs'][0]['config']['init']['x'] = [0, 0]
        ray_config['env_config']['env_objs'][0]['config']['init']['y'] = [0, 0]
        
        # wingman
        ray_config['env_config']['env_objs'][1]['config']['init']['radius'] = [1000, 2000]
